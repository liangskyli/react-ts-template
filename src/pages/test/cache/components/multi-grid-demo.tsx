...
import { MultiGrid } from '@/components/core/components/virtual-grid';
...
const MultiGridDemo = () => {
  ...
  const multiGrid1Ref = useRef<MultiGridRef>(null);
  const virtualGridCache = useCreateLRUCache<
    string,
    Parameters<Required<MultiGridProps>['getPositionCache']>[0]
  >('virtualGrid');
  const [virtualGridCacheValue, setVirtualGridCacheValue] =
    useState<Parameters<Required<MultiGridProps>['getPositionCache']>[0]>();
  ...
  const [virtualMultiGridCacheValue, setVirtualMultiGridValue] =
    useState<Parameters<Required<MultiGridProps>['getPositionCache']>[0]>();
  const multiGrid2Ref = useRef<MultiGridRef>(null);
  ...
  return (
    <div className="space-y-8 p-6">
      <h1 className="text-3xl font-bold text-gray-900">MultiGrid 控件演示</h1>
